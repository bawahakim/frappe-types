...
class TypeGenerator:
    ...
    def _get_module_path(self, app_name: str, module_name: str) -> Optional[Path]:
        ...
        type_setting = next(
            (ts for ts in type_generation_settings if ts["app_name"] == app_name), None
        )
        if not type_setting:
            return None
    ...
    def _generate_type_definition_content(self, doctype: DocType, module_path: Path):
        ...
        for field in doctype.fields:
            ...
            field_def, import_stmt = self._get_field_type_definition(
    ...
    def _get_field_type_definition(
        ...
        return field.fieldname + self._get_required(
            field
        ) + ": " + field_type, import_statement
    ...
    def _is_valid_doctype(self, doctype: DocType) -> bool:
        ...
        if not type_generation_settings.get("include_custom_doctypes", False) and (
            doctype.custom
        ):
